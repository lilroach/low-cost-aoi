...
def run_loop(points: List[Point]):
    ...
    try:
        ...
        if "run_dir" in job_state:
            ...
            program_path = os.path.join(job_state["run_dir"], "program.json")
            with open(program_path, "w") as f:
                json.dump(job_state.get("program", {"points": [p.model_dump() for p in points]}), f, indent=2)
...
async def upload_run(run_id: str):
    ...
    program_path = os.path.join(run_dir, "program.json")
    if os.path.exists(program_path):
        with open(program_path, "r") as f:
            program_data = json.load(f)
    else:
        program_data = {"name": report_data.get("metadata", {}).get("program_name", ""), "points": []}

    bundle = create_run_bundle(run_id, Path(run_dir), report_data, program_data)
    upload_url = f"{TRAINING_HOST_URL}/api/datasets/import-run"

    try:
        response = requests.post(
            upload_url,
            files={"bundle": (f"{run_id}.zip", bundle, "application/zip")},
            timeout=60,
        )
        response.raise_for_status()
    except requests.RequestException as exc:
        raise HTTPException(status_code=502, detail=f"Training Host upload failed: {exc}") from exc

    return {"status": "success", "training_host": upload_url, "response": response.json()}
...
async def start_run(req: RunRequest, background_tasks: BackgroundTasks):
    ...
    job_state["program"] = {"name": "ad-hoc", "points": [p.model_dump() for p in req.points]}
...
@router.post("/run")
async def run_program(req: RunProgramRequest, background_tasks: BackgroundTasks):
    if job_state["is_running"]:
        return {"status": "error", "message": "Already running"}

    selected_program = program._load_from_disk(req.program_name)
    if selected_program is None and program.current_program.name == req.program_name:
        selected_program = program.current_program
    if selected_program is None:
        return {"status": "error", "message": "Program not found"}

    points = runtime_state.aligned_points or selected_program.points
    runtime_state.aligned_points = None
    ...
    job_state["program"] = selected_program.model_dump()

    background_tasks.add_task(run_loop, points)

    return {"status": "started", "program_name": req.program_name, "points": len(points)}
