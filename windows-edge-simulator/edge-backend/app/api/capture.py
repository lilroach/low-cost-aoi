...
def _load_simulated_frame(source_image_url: str):
    if not source_image_url.startswith("/sim-camera/"):
        raise HTTPException(status_code=400, detail="Only /sim-camera/ source images are allowed")

    frontend_base_url = os.getenv("AOI_EDGE_FRONTEND_BASE_URL", "http://edge-frontend").rstrip("/")
    response = requests.get(f"{frontend_base_url}{source_image_url}", timeout=10)
    response.raise_for_status()

    image_array = cv2.imdecode(np.frombuffer(response.content, dtype=np.uint8), cv2.IMREAD_COLOR)
    if image_array is None:
        raise HTTPException(status_code=400, detail="Failed to decode simulated source image")
    return image_array
...
@router.post("/snap")
async def snap_image(part_no: str = "UNKNOWN", batch_no: str = "UNKNOWN", model_id: str = "none", source_image_url: str | None = None):
    """
    ...
    try:
        ...
        # 2. Capture frame
        if source_image_url:
            frame = _load_simulated_frame(source_image_url)
        else:
            camera.flush_buffer()
