...
import { AlertTriangle, Brain, Camera, CheckCircle, ChevronLeft, ChevronRight, Download, Eye, Hash, Maximize2, Package, Trash2, X, XCircle } from 'lucide-react'
...
export default function CaptureView() {
    ...
    const [isAnalyzingVlm, setIsAnalyzingVlm] = useState(false)
    const [vlmAnalysis, setVlmAnalysis] = useState<{ model: string; response: string } | null>(null)
    const [simCameraFrames, setSimCameraFrames] = useState<SimCameraFrame[]>([])
    const [simCameraIndex, setSimCameraIndex] = useState(0)
    const [simCameraIntervalMs, setSimCameraIntervalMs] = useState(2200)
    ...
    const simCameraFrame = simCameraFrames[simCameraIndex] ?? null
    const cameraImageSrc = simCameraFrame?.src ?? '/api/camera/feed'
    ...
    const fetchSimCameraFrames = async () => {
        const res = await fetch('/sim-camera/manifest.json', { cache: 'no-store' })
        const data = await res.json()
        setSimCameraFrames(data.images ?? [])
        setSimCameraIntervalMs(data.intervalMs ?? 2200)
    }

    useEffect(() => {
        fetchModels().catch(() => setModels([{ id: 'none', name: 'No model', version: '-', enabled: false }]))
        fetchCaptures().catch(() => setCaptures([]))
        fetchSimCameraFrames().catch(() => setSimCameraFrames([]))
    }, [])
    ...
    useEffect(() => {
        if (simCameraFrames.length <= 1) return
        const timer = window.setInterval(() => {
            setSimCameraIndex((index) => (index + 1) % simCameraFrames.length)
        }, simCameraIntervalMs)
        return () => window.clearInterval(timer)
    }, [simCameraFrames.length, simCameraIntervalMs])
    ...
    const handleSnap = async () => {
        ...
        try {
            ...
            if (simCameraFrame) {
                params.set('source_image_url', simCameraFrame.src)
            }
    ...
    const analyzeSelectedCaptureWithVlm = async () => {
        if (!selectedCapture || isAnalyzingVlm) return
        setIsAnalyzingVlm(true)
        try {
            const res = await fetch('/api/vlm/analyze', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ image_url: selectedCapture.url }),
            })
            if (!res.ok) {
                const data = await res.json().catch(() => ({ detail: 'VLM analysis failed' }))
                throw new Error(data.detail ?? 'VLM analysis failed')
            }
            setVlmAnalysis(await res.json())
        } catch (error) {
            alert(error instanceof Error ? error.message : 'VLM analysis failed')
        } finally {
            setIsAnalyzingVlm(false)
        }
    }
            ...
            <div className="grid min-h-0 flex-1 gap-4 overflow-hidden lg:grid-cols-[minmax(0,2fr)_minmax(360px,1fr)]">
                <div className="relative flex min-h-[420px] items-center justify-center overflow-hidden rounded-2xl border border-zinc-800 bg-black shadow-2xl shadow-black/40 lg:min-h-0">
                    ...
                    <div className="absolute right-4 top-4 z-30 flex max-w-[calc(100%-2rem)] items-center gap-2 rounded-lg border border-white/10 bg-black/75 px-3 py-2 text-xs text-zinc-300 backdrop-blur">
                        <span className="font-bold text-white">{simCameraFrame ? 'Simulated Live' : 'Live Feed'}</span>
                        {simCameraFrame && (
                            <>
                                <span className="text-zinc-600">/</span>
                                <span className="truncate font-mono">{simCameraIndex + 1}/{simCameraFrames.length}</span>
                            </>
                        )}
                    </div>
                    {simCameraFrame && (
                        <div className="absolute bottom-6 left-6 z-50 flex max-w-[calc(100%-9rem)] items-center gap-2 rounded-xl border border-white/10 bg-black/75 p-2 backdrop-blur">
                            <div className="hidden min-w-0 flex-col px-1 sm:flex">
                                <span className="max-w-[260px] truncate font-mono text-xs text-zinc-200">{simCameraFrame.name}</span>
                                <span className="text-[10px] uppercase text-zinc-500">SIM CAMERA SOURCE</span>
                            </div>
                        </div>
                    )}
                    <button
                    ...
                    <div className="grid h-[86vh] w-full max-w-6xl overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 shadow-2xl lg:grid-cols-[1fr_380px]">
                        ...
                        <div className="flex min-h-0 flex-col border-l border-zinc-800">
                            ...
                            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
                                ...
                                <div className="min-h-[168px] rounded-lg border border-zinc-800 bg-zinc-950 p-4">
                                    <div className="mb-3 flex items-center justify-between gap-3">
                                        <span className="text-[10px] font-bold uppercase text-zinc-500">Vision Model</span>
                                        <button
                                            onClick={analyzeSelectedCaptureWithVlm}
                                            disabled={isAnalyzingVlm}
                                            className="inline-flex items-center gap-2 rounded-lg border border-blue-700 bg-blue-950/50 px-3 py-2 text-xs font-bold text-blue-200 hover:bg-blue-900/60 disabled:cursor-not-allowed disabled:opacity-50"
                                        >
                                            {isAnalyzingVlm ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-300/30 border-t-blue-200" /> : <Brain size={15} />}
                                            Analyze
                                        </button>
                                    </div>
                                    {vlmAnalysis ? (
                                        <div className="space-y-2">
                                            <div className="font-mono text-[10px] uppercase text-zinc-500">{vlmAnalysis.model}</div>
                                            <div className="whitespace-pre-wrap rounded-lg border border-zinc-800 bg-zinc-900 p-3 text-sm leading-relaxed text-zinc-200">
                                                {vlmAnalysis.response}
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="rounded-lg border border-dashed border-zinc-800 bg-zinc-900/50 p-3 text-xs leading-relaxed text-zinc-500">
                                            Run the local vision model to get a general-language inspection note for this image.
                                        </div>
                                    )}
                                </div>
