...
| 模組 | 角色 | 原理 |
| :--- | :--- | :--- |
| Raspberry Pi Edge | 現場檢測端 | 執行 FastAPI backend、Web UI、相機取像、資料打包與後續 Hailo 推論。 |
| 相機與光源 | 影像來源 | 以固定曝光、固定解析度與穩定光源取得 PCB 局部影像，優先避免動態拍攝變形與反光干擾。 |
| Training Host | 訓練端 | 接收 Edge 回傳圖片與人工 OK / NG 結果，建立資料集並訓練 YOLO 類模型。 |
| 模型包 | 部署單位 | 以完整 model bundle 管理 `manifest.json`、模型檔、類別與 checksum，避免裸模型檔散落。 |
| 移動平台 | 自動掃描端 | Phase 4 才導入 CoreXY / Klipper，執行「移動 -> 拍照 -> 推論 -> 記錄」流程。 |
...
| 目標 | 預計效果 | 驗收方式 |
| :--- | :--- | :--- |
| 低成本建置 | 以約 NT$18,000 到 NT$23,500 完成含 Pi、相機、邊緣 AI、基礎運動控制的第一版硬體估算；若採較正式 USB3 工業相機，預算上緣需上修。 | BOM 與採購單比對。 |
| 現場取像 | Raspberry Pi 可直接拍照、預覽、保存圖片與記錄 PART / BATCH。 | Capture UI、`/api/camera/status`、snap 檔案驗證。 |
| 人工覆判與資料回收 | Phase 2 可在未啟用 AI 前先累積 OK / NG 樣本。 | Export / Transfer ZIP 可被 Training Host 匯入。 |
| 邊緣辨識 | Phase 3 導入 Hailo / YOLO 後，拍照後回傳 OK / NG 與缺陷資訊。 | 模型包安裝、啟用、推論結果與人工覆判比對。 |
| 自動化掃描 | Phase 4 導入 CoreXY 後，支援多點位掃描與 run bundle 報告。 | 掃描路徑、座標、圖片、結果 JSON 完整保存。 |
...
| 項目 | 建議規格 | 最新查詢價格 | 更新估算 | 備註 |
| :--- | :--- | :--- | :--- | :--- |
| SBC 主板 | Raspberry Pi 5 4GB | RS Taiwan 含稅 NT$4,394.25；勝特力 NT$4,999 | NT$4,400 到 NT$5,000 | 舊估 NT$3,200，需上修。 |
| 散熱 | Raspberry Pi 5 Active Cooler | 莓亞 NT$238；米羅 NT$240 起 | NT$240 | 維持主動散熱。 |
| 電源 | 官方 27W USB-C PD | 莓亞 NT$458 | NT$460 | Pi 5 建議供電規格。 |
| 邊緣 AI | Raspberry Pi AI HAT+ 13 TOPS | 官方起價 US$70 | 約 NT$2,200 | Phase 3 選配；AI Kit 已標示停產，新客戶應改買 AI HAT+。 |
| 下位機 | BTT SKR Pico V1.0 | BIQU 官方 US$29.89 | 約 NT$940 | Phase 4 用於 Klipper / CoreXY。 |
| 預算型全域快門相機 | Waveshare IMX296 Global Shutter | US$60.95 到 US$62.99 | 約 NT$1,920 到 NT$1,980 | 低成本候選，需確認介面與線長限制。 |
| Raspberry Pi 官方全域快門相機 | Raspberry Pi Global Shutter Camera | Waveshare US$79.31 到 US$79.99 | 約 NT$2,500 | CSI 生態成熟，但移動龍門需處理排線或 Pi 隨相機移動問題。 |
| USB3 工業 IMX296 相機 | InnoMaker U3V-CAM-IMX296 | 特價 US$179 | 約 NT$5,630 | 較符合相機與 Pi 分離、拖鏈走線與工業觸發需求。 |

### 更新後硬體預算區間

| 版本 | 內容 | 預估總額 |
| :--- | :--- | :--- |
| Phase 2 實機截圖版 | Pi 5 4GB、散熱、電源、既有 USB camera 或低成本全域快門相機、儲存與基本線材；不含 AI HAT+ 與移動平台。 | 約 NT$8,000 到 NT$11,000 |
| Phase 3 邊緣辨識版 | Phase 2 + AI HAT+ 13 TOPS + 模型包部署流程。 | 約 NT$10,200 到 NT$13,500 |
