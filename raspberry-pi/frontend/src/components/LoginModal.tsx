...
export const LoginModal = React.memo(() => {
    const { showLogin, setShowLogin, setUserRole, setTab } = useApp()
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
            <div className="bg-zinc-900 border border-zinc-700 p-8 rounded-2xl w-full max-w-sm shadow-2xl space-y-6">
                ...
                <div className="space-y-3">
                    <button onClick={() => { setUserRole('operator'); setShowLogin(false); setTab('run') }} className="w-full py-4 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-xl flex items-center justify-between px-6 group transition-all">
                    ...
                    <button onClick={() => { setUserRole('engineer'); setShowLogin(false) }} className="w-full py-4 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/30 hover:border-blue-500/50 rounded-xl flex items-center justify-between px-6 group transition-all">
